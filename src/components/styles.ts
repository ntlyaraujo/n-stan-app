/**
 * The class strings every screen shares.
 *
 * These were written out once per feature folder, and the focus ring five times
 * more besides, which is how a button on one screen quietly stops matching the
 * same button on another. One definition each, here.
 *
 * Every colour goes through a token — `bg-surface-raised`, `text-text-muted`,
 * `border-danger`. Tailwind's default palette is reset to `initial`, so a raw
 * palette colour such as `bg-white` silently produces no CSS at all.
 */
